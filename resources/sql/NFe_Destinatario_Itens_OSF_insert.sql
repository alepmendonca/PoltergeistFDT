/**
 * Insere depois Itens Destinatario
 */
...
SELECT
  ...
  ,CASE WHEN LENGTH(Quantidade) = 0 THEN 0.0 ELSE REPLACE(REPLACE(Quantidade, '.', ''), ',', '.') :: NUMERIC(14,4) END
  ,TRIM(Unidade)
  ...
  ,UF_Desembaraco
  ,Descricao_Texto
  ,CASE WHEN length(CPF_Emit) = 0 THEN NULL ELSE REGEXP_REPLACE(CPF_Emit, '[^0-9]+', '', 'g') END
  ,CASE 
  	WHEN LENGTH(TRIM(CNAE)) = 0 OR NOT EXISTS (SELECT 1 FROM CNAE WHERE CODIGO = REGEXP_REPLACE(TRIM(CNAE), '[^0-9]+', '', 'g')::INTEGER) THEN NULL 
  	ELSE REGEXP_REPLACE(TRIM(CNAE), '[^0-9]+', '', 'g')::INTEGER END
  ,CASE WHEN LENGTH(Codigo_Tributacao) = 0 THEN 0 ELSE Codigo_Tributacao::INTEGER END
  ,CASE WHEN LENGTH(Origem_Mercadoria) = 0 THEN NULL ELSE Origem_Mercadoria::INTEGER END
  ,TO_NUMBER(CASE WHEN LENGTH(TRIM(Percent_Aliq_Cred_SN)) = 0 THEN '0' ELSE TRIM(Percent_Aliq_Cred_SN) END, 'FM999G999G999D99')
  ,TO_NUMBER(CASE WHEN LENGTH(TRIM(Valor_Cred_SN)) = 0 THEN '0' ELSE TRIM(Valor_Cred_SN) END, 'FM999G999G999D99')
FROM NFE_ITEM_DEST_TEMP
